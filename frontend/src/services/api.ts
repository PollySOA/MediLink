...
export function resolveIdoniaOpenUrl(access: IdoniaAccessResponse, apiBase = BASE): string {
  const rawMagic = (access.magic_link_url ?? "").trim()
  if (/^https?:\/\//i.test(rawMagic)) return rawMagic

  const baseMagic = (access.magic_link_base_url ?? "").trim()
  if (baseMagic) {
    const separator = baseMagic.includes("?") ? "&" : "?"
    if (access.magic_link_route_urlsafe) {
      return `${baseMagic}${separator}url=${access.magic_link_route_urlsafe}`
    }
    if (access.magic_link_route) {
      return `${baseMagic}${separator}url=${encodeURIComponent(access.magic_link_route)}`
    }
    return baseMagic
  }

  return `${apiBase}${access.open_path}`
}
